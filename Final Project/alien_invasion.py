...
#Overall class to manage game assets and behavior
    ...
    def _draw_stats(self, num_lives, aliens_destroyed):
        green = (0,255,0)
        blue = (0,0,128)
        font = pygame.font.Font('freesansbold.ttf', 32)

        lives = font.render('Lives: {0}'.format(num_lives), 1, green, blue)
        livesRect = lives.get_rect()
        livesRect.center = (80, 20)
        self.screen.blit(lives, livesRect)

        aliens = font.render('Aliens Destroyed: {0}'.format(aliens_destroyed), 1, green, blue)
        aliensRect = lives.get_rect()
        aliensRect.center = (1250, 20)
        self.screen.blit(aliens, aliensRect)

        if self.stats.game_active == False:
            self.aliens.empty()
            restart = font.render('Restart? Press (Y)es or (N)o ', 1, green, blue)
            restartRect = restart.get_rect()
            restartRect.center = (self.settings.screen_width//2, self.settings.screen_height//2)
    ...
    def _check_events(self):
        ...
        for event in pygame.event.get():
            ...
            # Did the player press an arrow key?
            elif event.type == pygame.KEYDOWN:
                self._check_keydown_events(event)

            # Did the player stop holding down an arrow key?
    ...
    def _check_keydown_events(self, event):
        ...
        elif self.stats.game_active == False and event.key == pygame.K_y:
            self._restart()

        # Did the player hit the Q key to quite the game?
        elif event.key == pygame.K_q or (self.stats.game_active == False and event.key == pygame.K_n):
            sys.exit()
    ...
    def _update_screen(self):
        ...
        self.ship.blitme()
        self._draw_stats(self.stats.ships_left, self.stats.aliens_destroyed)
